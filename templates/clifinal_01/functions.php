...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    function artxPost($data)
    {
    	...
    	artxFragmentBegin(str_replace('class="kvm-box kvm-post">', 'class="kvm-box kvm-post' . $classes . '">', "<div class=\"kvm-box kvm-post\">\r\n    <div class=\"kvm-box-body kvm-post-body\">\r\n<div class=\"kvm-post-inner\">\r\n"));
    	artxFragmentBegin("<h2 class=\"kvm-postheader\">");
    	artxFragmentBegin("");
...
?>
        <?php ob_start(); ?>
<div class="kvm-box kvm-block">
            <div class="kvm-box-body kvm-block-body">
        
        <?php echo str_replace('class="kvm-box kvm-block">', 'class="kvm-box kvm-block' . $classes . '">', ob_get_clean()); ?>
        <?php if ($hasCaption): ?>
<div class="kvm-bar kvm-blockheader">
            <h3 class="t">
        <?php echo $caption; ?>
</h3>
        </div>
        <?php endif; ?>
        <?php if ($hasContent): ?>
<div class="kvm-box kvm-blockcontent">
            <div class="kvm-box-body kvm-blockcontent-body">
...
<?php
    ...
    function artxModules(&$document, $position, $style = null)
    {
         ...
         *  Select inputs with class != 'kvm-button'
         *   artxFindTags($html, array('input' => array('class' => '!kvm-button')))
         *  Select inputs with non-empty class
         *   artxFindTags($html, array('input' => array('class' => '!')))
         *  Select inputs with class != 'kvm-button' and non-empty class:
         *   artxFindTags($html, array('input' => array('class' => array('!kvm-button', '!'))))
         *  Select inputs with class = button but != 'kvm-button'
         *   artxFindTags($html, array('input' => array('class' => array('button', '!kvm-button'))))
         *
        ...
        function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
        {
            ...
            foreach (artxFindTags($content, $filter) as $tag) {
                if (is_string($tag))
                    ...
                    $tag['attributes']['class']['value'][] = 'kvm-button';
                    $delimeter = '' == $tag['attributes']['class']['delimeter']
                        ? '"' : $tag['attributes']['class']['delimeter'];
                    $tag['attributes']['class']['delimeter'] = $delimeter;
                    $button = str_replace('"', $delimeter, '<span class="kvm-button-wrapper">'
                        . '<span class="kvm-button-l"> </span><span class="kvm-button-r"> </span>')
        ...
        {
            return '<span class="kvm-button-wrapper"><span class="kvm-button-l"> </span><span class="kvm-button-r"> </span>'
                . '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
                . 'kvm-button" href="' . $data['link'] . '">' . $data['content'] . '</a></span>';
        }
...
function artxPostprocessBlockContent($content)
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!kvm-button')),
            'button' => array('class' => array('button', '!kvm-button'))));
        return $content;
    }
