...
class TZ_PortfolioModelArticle extends JModelAdmin
{
    ...
    protected function populateState(){
        ...
        if($params -> get('tz_image_xsmall')){
            $sizeImage['XS'] = (int) $params -> get('tz_image_xsmall');
        }
        if($params -> get('tz_image_small')){
            $sizeImage['S'] = (int) $params -> get('tz_image_small');
        }
        if($params -> get('tz_image_medium')){
            $sizeImage['M'] = (int) $params -> get('tz_image_medium');
        }
        if($params -> get('tz_image_large')){
            $sizeImage['L'] = (int) $params -> get('tz_image_large');
        }
        if($params -> get('tz_image_xsmall')){
            $sizeImage['XL'] = (int) $params -> get('tz_image_xlarge');
        }
		...
		while (!empty($pks))
		{
            ...
            foreach($rows as $row){
                ...
                if(!empty($row -> images)){
                    ...
                    $path   = JPATH_SITE.DS.str_replace('/',DS,$subUrl).DS.$imageName;
                    if(JFile::exists(JPATH_SITE.DS.str_replace('/',DS,$row -> images))){
                        JFile::copy(JPATH_SITE.DS.$row -> images,$path);
                    }
                }
            ...
            foreach($rows as $row){
                ...
                if(!empty($row -> attachfiles)){
                    ...
                    $attachFileTitle    = explode('///',$row -> attachtitle);
                    $i=0;
                    foreach($attachFileName as $item){
                        ...
                        if($attachFileTitle[$i] == $_fileName[count($_fileName) - 1]){
                            $attachTitle[$i]    = $fileName;
                        }
                        else
                            $attachTitle[$i]    = $attachFileTitle[$i];
                ...
                if(is_array($attachFiles) && count($attachFiles)>0){
                    ...
                    $attachTitle    = implode('///',$attachTitle);
                }
                //end attachment
                ...
                if(!empty($row -> images_hover)){
                    ...
                    foreach($sizes as $key => $val){
                        $newImageHover  = str_replace('/',DS,$imageHoverName);
                        $newImageHover  = str_replace('.'.JFile::getExt($row -> images_hover),
                                                  '_'.$key.'.'.JFile::getExt($row -> images_hover),$newImageHover);
                        $path       = JPATH_SITE.DS.$newImageHover;

                        $srcName    = str_replace('/',DS,$row -> images_hover);
                        $srcName    = str_replace('.'.JFile::getExt($srcName),
                                                  '_'.$key.'.'.JFile::getExt($srcName),$srcName);
                        $srcPath2   = JPATH_SITE.DS.$srcName;
                        if(JFile::exists($srcPath2)){
                ...
                if(!empty($row -> gallery)){
                    ...
                    foreach($arr as $gallery){
                        ...
                        foreach($sizes as $key => $val){
                            $newGallery   = str_replace('/',DS,$subUrl.'/'.$str);
                            $newGallery    = str_replace('.'.JFile::getExt($newGallery),
                                                       '_'.$key.'.'.JFile::getExt($gallery),$newGallery);
                            $path       = JPATH_SITE.DS.$newGallery;

                            $srcName    = str_replace('/',DS,$gallery);
                            $srcName    = str_replace('.'.JFile::getExt($srcName),
                                                      '_'.$key.'.'.JFile::getExt($srcName),$srcName);
                            $srcPath2   = JPATH_SITE.DS.$srcName;
                            if(JFile::exists($srcPath2)){
                        ...
                        if(JFile::exists(JPATH_SITE.DS.str_replace('/',DS,$gallery))){
                            JFile::copy(JPATH_SITE.DS.str_replace('/',DS,$gallery),$path);
                        }
                    }
                ...
                if(!empty($row -> videothumb)){
                    ...
                    foreach($sizes as $key => $val){
                        $newThumb   = str_replace('/',DS,$videoThumb);
                        $newThumb    = str_replace('.'.JFile::getExt($newThumb),
                                                   '_'.$key.'.'.JFile::getExt($newThumb),$newThumb);
                        $path       = JPATH_SITE.DS.$newThumb;

                        $srcName    = str_replace('/',DS,$row -> videothumb);
                        $srcName    = str_replace('.'.JFile::getExt($srcName),
                                                  '_'.$key.'.'.JFile::getExt($srcName),$srcName);
                        $srcPath2   = JPATH_SITE.DS.$srcName;
                        if(JFile::exists($srcPath2)){
    ...
    $title  = array();
        ...
        foreach($arr as $item){
            if($this -> tzfolder.'/'.$this -> attachUrl.'/'.$obj_json -> attachmentsFile != $item)
                ...
                // Delete file
                $filePath   = JPATH_SITE.DS.'media'.DS.$this -> tzfolder.DS.$this-> attachUrl.DS.$obj_json -> attachmentsFile;
                    ...
                    //foreach($rows as $row){
                        ...
                        if(preg_match('/.*\/\/\/.*/i',$rows -> attachfiles,$match)){
                            ...
                            foreach($attachFiles as $item){
                                $filePath   = JPATH_SITE.DS.$folder.DS.$item;
                                if(JFile::exists($filePath)){
                                    JFile::delete($filePath);
                                }
                            }
                            ...
                            $filePath   = JPATH_SITE.DS.$folder.DS.$rows -> attachfiles;
                            if(JFile::exists($filePath)){
                                JFile::delete($filePath);
                            }
                        }
...
//        $arr['row']     = $this -> getFieldsContent();
    ...
    public function renderFields($groupId=null,$catid=null){
        ...
        if(count($rows)>0){
            ...
            require_once(JPATH_COMPONENT_ADMINISTRATOR.DS.'helpers'.DS.'article.php');
...
//        $artid          = JRequest::getInt('id',null);
	...
	*/
            ...
            foreach($rows as $item){
                ...
                foreach($sizes as $key => $size){
                    ...
                    if(!empty($item -> images)){
                        ...
                        $path   = JPATH_SITE.DS.str_replace('/',DS,$str);
                    ...
                    if(!empty($item -> images_hover)){
                        ...
                        $str4    = str_replace('/',DS,$str4);
                        $path4   = JPATH_SITE.DS.str_replace('/',DS,$str4);
                    ...
                    if(!empty($item -> gallery)){
                        ...
                        foreach($gallerys as $gallery){
                            ...
                            $path2   = JPATH_SITE.DS.str_replace('/',DS,$str2);
                    ...
                    if(!empty($item -> videothumb)){
                        ...
                        $path3   = JPATH_SITE.DS.str_replace('/',DS,$str3);
    ...
    function filterTagsName($tagsName,$articleId){
...
//        $tagsName   = str_replace(' ','',$tagsName);
        $tagsIds    = array();
        $nameFiltered   =  $this -> filterTagsName($tagsName,$articleId);

        $this -> insertTagsNew($nameFiltered);

        if($tagsName)
            $tagsIds    = $this -> getTagsId($tagsName);
    ...
    function getImageHover($fileClient,$fileServer = null,$data=null,$task=null){
        ...
        if($fileClient){
            if(!empty($fileClient['name'])){
                ...
                foreach($sizes as $key => $newWidth){
                    ...
                    if(isset($curfile) && !empty($curfile)){
                        ...
                        $_curFile    = JPATH_SITE.DS.$curName;
                    ...
                    //Upload Image
                    $destPath   = JPATH_SITE.DS.str_replace('/',DS,$str);
            ...
            elseif(!empty($fileServer)){
                // Check size
                $originalFile   = JPATH_SITE.DS.str_replace('/',DS,$fileServer);
                ...
                if(isset($curfile) && !empty($curfile)){
                    ...
                    if($task == 'save2copy'){
                        ...
                        foreach($sizes as $key => $val){
                            ...
                            $curPath    = JPATH_SITE.DS.str_replace('/',DS,$curPath);

                            $destPath2  = JPATH_SITE.DS.str_replace('/',DS,$str2);
...
//                    foreach($fileServer as $i => $item){
                if(!empty($fileServer[$i])){
                    $original   = JPATH_SITE.DS.str_replace('/',DS,$fileServer[$i]);
...
//                    }
    ...
    function getVideo($data = null,$task = null){
        ...
        if(!JFile::exists($destPath.DS.'index.html')){
            JFile::write($destPath.DS.'index.html',htmlspecialchars_decode('<!DOCTYPE html><title></title>'));
        }
        if(!JFolder::exists($destPath.DS.'thumbnail')){
            JFolder::create($destPath.DS.'thumbnail');
        }
        if(!JFile::exists($destPath.DS.'thumbnail'.DS.'index.html')){
            JFile::write($destPath.DS.'thumbnail'.DS.'index.html',htmlspecialchars_decode('<!DOCTYPE html><title></title>'));
        }
        $destPath   .= DS.'thumbnail';
            ...
            switch ($data['tz_media_type']){
                default:
                    ...
                    if($data['tz_media_code']){
                        ...
                        if(JFile::exists(JPATH_SITE.DS.str_replace('/',DS,$data['tz_thumb']))){
                            ...
                            $url  = JPATH_SITE.DS.str_replace('/',DS,$data['tz_thumb']);
                            $obj    = new JImage($url);
                            $width  = $obj -> getWidth();
                            $height = $obj -> getHeight();
                            foreach($size as $key => $newWidth){
                ...
                case 'youtube':
                    ...
                    if(!empty($data['tz_media_code_youtube'])){
                        ...
                        if(!JFolder::exists($destPath.DS.'youtube')){
                            JFolder::create($destPath.DS.'youtube');
                        }
                        if(!JFile::exists($destPath.DS.'youtube'.DS.'index.html')){
                            JFile::write($destPath.DS.'youtube'.DS.'index.html',htmlspecialchars_decode('<!DOCTYPE html><title></title>'));
                        }

                        //Upload Image
                        if(!JFile::exists($destPath.DS.'youtube'.DS.$_fileName)){
                            if($thumb){
                                if(JFile::write($destPath.DS.'youtube'.DS.$_fileName,$thumb -> body)){
                                    $url = $destPath.DS.'youtube'.DS.$_fileName;
                                }
                            }
                        }
                        ...
                        if(isset($url) && !empty($url)){
                            ...
                            foreach($size as $key => $newWidth){
                                ...
                                $_destPath   = JPATH_SITE.DS.str_replace('/',DS,$str);
                                $type       = $this -> _getImageType($str);
                                $newImage -> toFile($_destPath,$type);
                            }
                ...
                case 'vimeo':
                    ...
                    if(!empty($data['tz_media_code_vimeo'])){
                        ...
                        if(!JFolder::exists($destPath.DS.'vimeo')){
                            JFolder::create($destPath.DS.'vimeo');
                        }
                        if(!JFile::exists($destPath.DS.'vimeo'.DS.'index.html')){
                            JFile::write($destPath.DS.'vimeo'.DS.'index.html',htmlspecialchars_decode('<!DOCTYPE html><title></title>'));
                        }

                        if(!JFile::exists($destPath.DS.'vimeo'.DS.$_fileName)){
                            if($thumb){
                                if(JFile::write($destPath.DS.'vimeo'.DS.$_fileName,$thumb -> body)){
                                    $url = JPATH_SITE.DS.$this -> imageUrl.'/cache/thumbnail/vimeo/'.$_fileName;
                                }
                            }
                        }
    ...
    function deleteThumb($articleId=null,$file=null){
        ...
        if($file){
            ...
            if($row   = $db -> loadObject()){
                if(!empty($row -> videothumb)){
                    $file   = JPATH_SITE.DS.$row -> videothumb;
