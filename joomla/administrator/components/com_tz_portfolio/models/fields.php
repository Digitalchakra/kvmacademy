...
require_once(JPATH_COMPONENT.DS.'tables'.DS.'fields.php');

class TZ_PortfolioModelFields extends JModel
{
    public $_link   = null;
    public $_task   = null;
    public $msg     = null;
...
//                  .' GROUP BY t.contentid';
    ...
    public function getFields(){
        ...
        $order          = (!in_array($order,
            array('f.id','groupname','f.name','f.type','f.published')))?
            'f.ordering':($order);
        ...
        foreach($rows as $row){
            ...
            $query1       = 'SELECT DISTINCT fg.name FROM #__tz_portfolio_fields AS f'
                          .' INNER JOIN #__tz_portfolio_xref AS x'
                          .' ON f.id=x.fieldsid'
                          .' LEFT JOIN #__tz_portfolio_fields_group AS fg'
                          .' ON fg.id=x.groupid'
