...
class TZ_PortfolioModelPortfolio extends JModel
{
    ...
    function getArticle(){
        ...
        $query  = 'SELECT c.*,t.name AS tagName,cc.title AS category_title,'
        ...
        if(count($rows)>0){
            foreach($rows as $key => $item){
                ...
                if($params -> get('tz_article_intro_limit',20)){
                    $intro  = strip_tags($rows[$key] -> introtext);
                    $intro  = explode(' ',$intro);
                    $rows[$key] -> introtext    = implode(' ',array_splice($intro,0,$params -> get('tz_article_intro_limit',20)));
                }
