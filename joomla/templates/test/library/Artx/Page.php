...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="kvm-content-layout">';
        $result .= '<div class="kvm-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="kvm-layout-cell' . ('kvm-block' == $style ? ' kvm-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="kvm-post">' string in it.
     * Then it wraps the content of the buffer with the kvm-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="kvm-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'kvm-main').addClass('kvm-postcontent');
            }
        });
    }
});
