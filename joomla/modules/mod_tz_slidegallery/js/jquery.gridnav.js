(function(jQuery) {
			...
			init 	: function( options ) {
				...
				if( this.length ) {
					...
					return this.each(function() {
						...
						var el 			= jQuery(this).css( 'visibility', 'hidden' ),
							// the ul
							...
							p_nav			= jQuery(settings.navL),
							n_nav			= jQuery(settings.navR);
	...
	jQuery.fn.gridnav = function(method) {
		if ( methods[method] ) {
			return methods[method].apply( this, Array.prototype.slice.call( arguments, 1 ));
		} else if ( typeof method === 'object' || ! method ) {
			return methods.init.apply( this, arguments );
		} else {
			jQuery.error( 'Method ' +  method + ' does not exist on jQuery.gridnav' );
		}
	};
})(jQuery);		
