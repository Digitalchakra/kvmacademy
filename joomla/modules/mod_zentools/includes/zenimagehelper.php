		...
		foreach ($images as $image)
		{
			...
			if ($resizeImage) {
				if(!$responsiveimages) {
					...
					$image =  resizeImageHelper::getResizedImage($new_image, $img_width, $img_height, $option);
					$new_item->imageTiny = resizeImageHelper::getResizedImage($new_image, ($img_width /5), ($img_height / 5), $option);
					$new_item->imageXSmall = resizeImageHelper::getResizedImage($new_image, ($img_width /3), ($img_height / 3), $option);
					$new_item->imageSmall = resizeImageHelper::getResizedImage($new_image, ($img_width /2), ($img_height / 2), $option);
					$new_item->imageMedium = resizeImageHelper::getResizedImage($new_image, ($img_width /1.25), ($img_height / 1.25), $option);
					$new_item->imageDefault = resizeImageHelper::getResizedImage($new_image, $img_width, $img_height, $option);
					
					$new_item->imageLarge = resizeImageHelper::getResizedImage($new_image, ($img_width *1.25), ($img_height * 1.25), $option);
					if($new_item->imageLarge == $new_image) { 
						$new_item->imageLarge = $new_item->imageDefault;
					}
					$new_item->imageXLarge = resizeImageHelper::getResizedImage($new_image, ($img_width *1.75), ($img_height * 1.75), $option);
				...
				$image = $new_image;
					...
					if($responsiveimages) {
						...
						$new_item->imageTiny = resizeImageHelper::getResizedImage($new_image, ($width /5), ($height / 5), $option);
						$new_item->imageXSmall = resizeImageHelper::getResizedImage($new_image, ($width /3), ($height / 3), $option);
						$new_item->imageSmall = resizeImageHelper::getResizedImage($new_image, ($width /2), ($height / 2), $option);
						$new_item->imageMedium = resizeImageHelper::getResizedImage($new_image, ($width /1.5), ($height / 1.5), $option);
						$new_item->imageDefault = resizeImageHelper::getResizedImage($new_image, ($width), ($height), $option);
						$new_item->imageLarge = resizeImageHelper::getResizedImage($new_image, ($width *1.25), ($height * 1.25), $option);
						if($new_item->imageLarge == $new_image) { 
							$new_item->imageLarge = $new_item->imageDefault;
						}
						$new_item->imageXLarge = resizeImageHelper::getResizedImage($new_image, ($width *1.75), ($height * 1.75), $option);
			...
			if($layout == "slideshow") {
				$new_item->thumb = resizeImageHelper::getResizedImage($new_image, $thumb_width, $thumb_height,  $option);
			}
			...
			$new_item->title = $titlewordCount ? ZenHelper::truncate($titletext, $titlewordCount, $titleSuffix) : $titletext;
			$new_item->modaltitle = $titlewordCount ? ZenHelper::truncate($titletext, $titlewordCount, $titleSuffix) : $titletext;


			// Item Description
			$text = str_replace(''.$separator.'',' ', ''.$text.'');
			$new_item->text = $wordCount ? ZenHelper::truncate($text, $wordCount, $titleSuffix) : $text;
