...
class ModZentoolsHelper
{
	function getList($params, $id)
	{
		...
		$modalMore = $params->get('modalMore');
				...
				//Articles query
				$query = 'SELECT a.*, cc.title as catname, s.title as sectitle, ' .
			...
			if($lists[$i]->image) {
				...
				if ($resizeImage) {
					$lists[$i]->image =  resizeImageHelper::getResizedImage($lists[$i]->image, $img_width, $img_height, $option);
						...
						if($responsiveimages) {		
							$lists[$i]->imageTiny = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width /5), ($img_height / 5), $option);
							$lists[$i]->imageXSmall = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width /3), ($img_height / 3), $option);
							$lists[$i]->imageSmall = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width /2), ($img_height / 2), $option);
							$lists[$i]->imageMedium = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width /1.5), ($img_height / 1.5), $option);
							$lists[$i]->imageDefault = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width), ($img_height), $option);
							
							$lists[$i]->imageLarge = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width *1.25), ($img_height * 1.25), $option);
							if($lists[$i]->imageLarge == $lists[$i]->image) { 
								$lists[$i]->imageLarge = $lists[$i]->imageDefault;
							}
							
							$lists[$i]->imageXLarge = resizeImageHelper::getResizedImage($lists[$i]->image, ($img_width *1.75), ($img_height * 1.75), $option);
				...
				$lists[$i]->image = $lists[$i]->image;
						...
						if($responsiveimages) {	
							...
							if($lists[$i]->image !=="") {
								...
								$lists[$i]->imageTiny = resizeImageHelper::getResizedImage($lists[$i]->image, ($width /5), ($height / 5), 'exact');
								$lists[$i]->imageXSmall = resizeImageHelper::getResizedImage($lists[$i]->image, ($width /3), ($height / 3), 'exact');
								$lists[$i]->imageSmall = resizeImageHelper::getResizedImage($lists[$i]->image, ($width /2), ($height / 2), 'exact');
								$lists[$i]->imageMedium = resizeImageHelper::getResizedImage($lists[$i]->image, ($width /1.5), ($height / 1.5), 'exact');
								$lists[$i]->imageDefault = resizeImageHelper::getResizedImage($lists[$i]->image, ($width), ($height), 'exact');
								$lists[$i]->imageLarge = resizeImageHelper::getResizedImage($lists[$i]->image, ($width *1.25), ($height * 1.25), $option);
								if($lists[$i]->imageLarge == $lists[$i]->image) { 
									$lists[$i]->imageLarge = $lists[$i]->imageDefault;
								}
							
								$lists[$i]->imageXLarge = resizeImageHelper::getResizedImage($lists[$i]->image, ($width *1.75), ($height * 1.75), $option);
								if($lists[$i]->imageXLarge == $lists[$i]->image) { 
									$lists[$i]->imageXLarge = $lists[$i]->imageDefault;
								}
						}
