...
class plgSystemModulesAnywhereHelper
{
	...
	function processModules(&$string, $area = 'articles', $message = '')
	{
		...
		if (preg_match('#\{' . $this->params->tags . '#', $string)) {
			...
			self::replace($string, $this->params->regex);
			self::replace($string, $this->params->regex2);
		}
	}
	...
	function processMatch(&$string, &$match, &$message)
	{
		...
		if ($message != '') {
			...
			foreach ($vars as $var) {
				...
				if (strpos($var, '=') === false) {
					...
					if ($type == $this->params->module_tag) {
						...
						if ($key == 'style') {
							$chrome = $val;
						} else if (in_array($key, array('ignore_access', 'ignore_state', 'ignore_assignments'))) {
							$ignores[$key] = $val;
			...
			if ($type == $this->params->module_tag) {
				// module
				$html = $this->processModule($id, $chrome, $ignores, $overrides);
			} else {
				// module position
	...
	function processModule($module, $chrome = 'none', $ignores = array(), $overrides = array())
	{
		...
		$document->_type = 'html';
		$renderer = $document->loadRenderer('module');
		return $renderer->render($module, array('style' => $chrome, 'name' => ''));
	}
