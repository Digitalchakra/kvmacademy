...
class TZ_PortfolioViewTimeLine extends JView
{
    function display($tpl=null){
        ...
        if(count($list)>0){
            foreach($list as $key => $item){
                ...
                $results = $dispatcher->trigger('onContentPrepare', array ('com_tz_portfolio.portfolio', &$item, &$params, $state -> get('offset')));

                $item->event = new stdClass();
                $results = $dispatcher->trigger('onContentAfterTitle', array('com_tz_portfolio.portfolio', &$item, &$params, $state -> get('offset')));
                $item->event->afterDisplayTitle = trim(implode("\n", $results));

                $results = $dispatcher->trigger('onContentBeforeDisplay', array('com_tz_portfolio.portfolio', &$item, &$params, $state -> get('offset')));
                $item->event->beforeDisplayContent = trim(implode("\n", $results));

                $results = $dispatcher->trigger('onContentAfterDisplay', array('com_tz_portfolio.portfolio', &$item, &$params, $state -> get('offset')));
        ...
        $doc -> addStyleSheet('components/com_tz_portfolio/assets/tz_portfolio.css');
        ...
        if($params -> get('tz_timeline_layout') == 'default'):
            ...
            $doc -> addScript('components/com_tz_portfolio/js/blog.js');
        else:
            $doc -> addScript('components/com_tz_portfolio/js/jquery.infinitescroll.min.js');
            $doc -> addStyleSheet('components/com_tz_portfolio/css/style.css');
            $doc -> addScript('components/com_tz_portfolio/js/jquery.isotope.js');
            $doc -> addScript('components/com_tz_portfolio/js/html5.js');
        endif;
