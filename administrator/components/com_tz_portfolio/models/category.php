...
<?php
    ...
    public function getFieldsGroup(){
        ...
        if(!$rows2 = $dbo -> loadObjectList()){
            ...
            $fieldsgroup  .= '<option value="">'.JText::_('Select fields group').'</option>';
            $fieldsgroup  .= '</select>';
            return $fieldsgroup;
        }
...
//		$params = JComponentHelper::getParams('com_categories');
    ...
    function deleteImages($fileName){
        if($fileName){
            $file   = JPATH_SITE.DS.str_replace('/',DS,$fileName);
    ...
    function uploadImages($file,$url=null){
        if($file){
            ...
            $tzFolderPath       = JPATH_ROOT.DS.'media'.DS.$tzFolder;
            $tzUserFolderPath   = $tzFolderPath.DS.$tzUserFolder;

            if(!JFolder::exists($tzFolderPath)){
                JFolder::create($tzFolderPath);
                if(!JFile::exists($tzFolderPath.DS.'index.html')){
                    JFile::write($tzFolderPath.DS.'index.html'
            ...
            if(JFolder::exists($tzFolderPath)){
                if(!JFolder::exists($tzUserFolderPath)){
                    ...
                    if(!JFile::exists($tzUserFolderPath.DS.'index.html'))
                        JFile::write($tzUserFolderPath.DS.'index.html'
                            ,htmlspecialchars_decode('<!DOCTYPE html><title></title>'));
                }
            }
            $params     = JComponentHelper::getParams('com_tz_portfolio');
            if(!$url){
                ...
                $image      = new JImage(JPATH_SITE.DS.str_replace('/',DS,$file));

                $desFileName    = 'categories_'.time().uniqid().'.'.JFile::getExt($file);
                $desPath        = $tzUserFolderPath.DS.$desFileName;
                ...
                require_once(JPATH_COMPONENT.DS.'helpers'.DS.'HTTPFetcher.php');
                require_once(JPATH_COMPONENT.DS.'helpers'.DS.'readfile.php');
    ...
    function delete($pks){
        if($pks){
            ...
            if($rows = $db -> loadObjectList()){
                foreach($rows as $row){
                    if(!empty($row -> images)){
                        $path   = JPATH_SITE.DS.str_replace('/',DS,$row -> images);
